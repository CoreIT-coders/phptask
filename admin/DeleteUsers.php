...
//include Import File && and if the user not admin Redirect to Home=>Public Area
    ...
    require '../include/temp/admindashbord.php';
                //view the user in table with there information to delete them
?>
                <table class="table table-striped" style="margin-top: 27px;display: inline-table; width: 80%;">
                  ...
                  <tbody>
<?php
        //Select query to get all User from the DateBase
        $i=1;
        if(true)
            {
                $sql = "SELECT `id`,`email`, `username`, `Date` FROM `user` WHERE `admin`=0";
                ...
                while ($row=mysqli_fetch_assoc($result))
                {
                    ...
                    echo "<td>".$row['Date']."</td>";
                    echo "<td><a class='btn btn-danger' href='delete_submit.php?page=user&id=".$row['id']."'>Delete</a></td>";
                    echo "</tr>";
                    $i++;
                }
            }
